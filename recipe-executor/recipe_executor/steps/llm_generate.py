...
def _render_config(config: Dict[str, Any], context: ContextProtocol) -> Dict[str, Any]:
    """
    Recursively render templated strings in a dict using the context.
    """
    result: Dict[str, Any] = {}
...
class LLMGenerateStep(BaseStep[LLMGenerateConfig]):
    ...
    async def execute(self, context: ContextProtocol) -> None:
        ...
        if raw_max is not None:
            rendered_max = render_template(str(raw_max), context)
            try:
                max_tokens = int(rendered_max)
            except ValueError:
                raise ValueError(f"Invalid max_tokens value: {raw_max!r}")

        # Gather MCP server configs from step config and context config
        mcp_cfgs: List[Dict[str, Any]] = []
        if self.config.mcp_servers:
            mcp_cfgs.extend(self.config.mcp_servers)
        ctx_servers = context.get_config().get("mcp_servers") or []
        if isinstance(ctx_servers, list):
            mcp_cfgs.extend(ctx_servers)
