...
def _render_override(value: Any, context: ContextProtocol) -> Any:
    ...
    - Strings are template-rendered, then if the result is a valid literal (dict or list), parsed into Python objects using ast.literal_eval.
    - Lists and dicts are processed recursively.
    - Other types are returned as-is.
    """
    if isinstance(value, str):
        rendered = render_template(value, context)
        # Attempt to parse Python literal (dict or list)
        try:
...
class ExecuteRecipeStep(BaseStep[ExecuteRecipeConfig]):
    ...
    async def execute(self, context: ContextProtocol) -> None:
        ...
        # Apply context overrides with templating and literal evaluation
